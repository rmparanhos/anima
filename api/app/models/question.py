...
class Question(Base):
    ...
    original_message_id: Mapped[Optional[str]] = mapped_column(String, ForeignKey("messages.id"), nullable=True)
    normalized_text: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[str] = mapped_column(String(32), default="pending")  # pending | answered | rejected
    answer_text: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    answered_by: Mapped[Optional[str]] = mapped_column(String, ForeignKey("users.id"), nullable=True)
    answered_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    knowledge_chunk_id: Mapped[Optional[str]] = mapped_column(String, ForeignKey("knowledge_chunks.id"), nullable=True)
    votes: Mapped[int] = mapped_column(Integer, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=lambda: datetime.now(timezone.utc))

    asker: Mapped["User"] = relationship(foreign_keys=[asked_by], back_populates="questions_asked")
    answerer: Mapped[Optional["User"]] = relationship(foreign_keys=[answered_by], back_populates="questions_answered")
