...
export default function PendingPage() {
  ...
  const answerMutation = useMutation({
    mutationFn: ({ id, text }: { id: string; text: string }) => api.answerQuestion(id, text, userId!),
    onSuccess: () => {
    ...
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "calc(100vh - 48px)" }}>
        <p style={{ color: "#8888aa", fontSize: "14px" }}>
          Sign in at <Link to="/chat" style={{ color: "#5b5bd6", textDecoration: "underline" }}>chat</Link> first.
        </p>
      </div>
    )
  }
    ...
    <div style={{ maxWidth: "768px", margin: "0 auto", padding: "32px 16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        {data?.questions.map((q: Question) => (
          <div key={q.id} style={{ background: "#16161f", border: "1px solid #2a2a3a", borderRadius: "8px", padding: "20px" }}>
            ...
            {answering === q.id ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                <textarea
                  style={{ width: "100%", background: "#1e1e2e", border: "1px solid #2a2a3a", borderRadius: "6px", padding: "8px 12px", fontSize: "14px", color: "#e2e2e9", outline: "none", resize: "none", lineHeight: 1.6 }}
                  rows={4}
                ...
                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                  <button
                    ...
                    style={{ background: "#5b5bd6", color: "#fff", border: "none", borderRadius: "6px", padding: "7px 14px", fontSize: "14px", fontWeight: 600, cursor: !answerText.trim() || answerMutation.isPending ? "not-allowed" : "pointer", opacity: !answerText.trim() || answerMutation.isPending ? 0.5 : 1 }}
                  >
